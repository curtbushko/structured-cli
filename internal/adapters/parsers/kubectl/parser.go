...
// Common constants for kubectl output parsing.
const (
	noneValue    = "<none>"
	pendingValue = "<pending>"

	// Common command, subcommand and column names.
	cmdKubectl       = "kubectl"
	cmdGet           = "get"
...
// parseInput is a generic function type for parsing kubectl tabular output.
...
func readAndPrepare(r io.Reader, colNames []string, requiredCols []string) parseResult {
	...
	// Check if this looks like valid kubectl output
...
func parseHeaderColumns(header string, colNames []string) []columnInfo {
	...
	// Sort columns by start position using bubble sort
...
// This prevents "NAME" from matching within "NAMESPACE".
...
// normalizeKubectlValue normalizes kubectl placeholder values like <none> and <pending>.
func normalizeKubectlValue(value string) string {
	if value == noneValue || value == pendingValue {
		return ""
	}
	return value
}
